import { Children, MutableRefObject, useState, useRef } from "react";
import AnimatedBackground from "./motion/motionbackround";
import CustomPatternSVG from "./motion/motiondraw";
import { TypeAnimation } from 'react-type-animation';

export default function Container(props: {
  [x: string]: any; children: any; ref1: MutableRefObject<HTMLDivElement | null>;
  ref2: MutableRefObject<HTMLDivElement | null>; ref3: MutableRefObject<HTMLDivElement | null>
}) {
  ...
  const handleClickLocation = () => { props.ref3.current?.scrollIntoView({ behavior: 'smooth' }); };
    ...
    <div className="relative z-10 flex flex-col pt-[40px] h-full w-full overflow-x-hidden overflow-y-auto">
      ...
      <main id="skip" className="flex-1 w-full p-4">
        ...
        <div className="sticky right-0 top-0 w-12 sm:w-20 flex flex-col items-start space-y-12 ml-auto z-50">
          <button className="w-16 h-12 text-[#ddd8b9] font-extrabold font-special	text-2xl transform rotate-90" onClick={handleClickFaq} >
            <TypeAnimation
          ...
          <button className="w-16 h-44 text-[#ddd8b9] font-extrabold font-special	text-2xl transform rotate-90" onClick={handleClickTickets}>
            {show1 && <TypeAnimation
          ...
          <button className="w-16 h-72 text-[#ddd8b9] font-extrabold font-special	text-2xl transform rotate-90" onClick={handleClickLocation}>
            {show2 && <TypeAnimation
